...
GetSILSData <- function(StartWeek, EndWeek) {
  ...
  SILSData <- SILSData[, .(SCLS = max(SCLS), SRLS = max(SRLS), SIPLS = max(SIPLS), age = min(age)), keyby = .(cprnr, ISOweek = ISOweek::ISOweek(as.Date(date)))]
  ...
  SILSData <- rbind(SILSData,
                    SILSData[(65 <= age) & !is.na(age), .(cprnr, ISOweek, SCLS, SIPLS, SRLS, age, group = "65P")],
                    SILSData[, .(cprnr, ISOweek, SCLS, SIPLS, SRLS, age, group = "Total")])

  SILSData <- SILSData[!is.na(group), .(SCLS = sum(SCLS), SIPLS = sum(SIPLS), SRLS = sum(SRLS)), keyby = .(group, ISOweek)]
