...
GetVirData <- function(StartWeek, EndWeek) {
  library(data.table)
  source("R/GetDoB.R", encoding = "UTF-8")

  # COVID-19 tests
  source("R/GetBasicSamplesCOVID19.R", encoding = "UTF-8")
  COVID19 <- data.table(GetBasicSamplesCOVID19(ISOweek::ISOweek2date(paste0(StartWeek, "-1")), ISOweek::ISOweek2date(paste0(EndWeek, "-7"))))
  # Influenza tests
  source("R/EpiMiBaDataCovid19.R", encoding = "UTF-8")
  MiBaData <- data.table(GetMiBaData(ISOweek::ISOweek2date(paste0(StartWeek, "-1")), ISOweek::ISOweek2date(paste0(EndWeek, "-7"))))[, .(cprnr, prdate, A, B)]
  MiBaData[!(is.na(A) & is.na(B)), .(cprnr, prdate = substr(prdate,1,10), A, B)]

  MiBaData <- merge(MiBaData, COVID19, by = c("cprnr", "prdate"), all = TRUE)

  MiBaData[, `:=`(age = as.integer(floor((as.Date(prdate) - GetDoB(cprnr))/365.25)),
                  ISOweek = ISOweek::ISOweek(as.Date(prdate)))][, prdate := NULL]
  ...
  MiBaData <- rbind(MiBaData,
                    MiBaData[(65 <= age) & !is.na(age), .(cprnr, A, B, COVID19, age, ISOweek, group = "65P")],
                    MiBaData[, .(cprnr, A, B, COVID19, age, ISOweek, group = "Total")])
